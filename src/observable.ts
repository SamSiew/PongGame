...
class SafeObserver<T> implements Observer<T> {
  ...
  /**
   * 
   * @param errors: Error object appears
   * @returns None: a void function 
   * throw Error Message to console when error appears 
   * to allow programmer and user knows what type of error encountered
   */
  error(errors:Error):void {
      throw("Error appear"+errors)
  }
}
...
class Observable<Input> {
   ...
   * throw any error to console if error found.
  ...
  subscribe(next:(_:Input)=>void, complete?: ()=>void): ()=>void {
    const safeObserver = new SafeObserver(<Observer<Input>>{
        ...
        error: (errors:Error):void => {throw('Error Found:'+ errors.message)}
      });
  ...
  static fromArray<V>(arr: V[]):Observable<V> {
      if(arr == undefined||arr.length==0) throw new Error('NullArray:cannot access empty Array')
  ...
  map<R>(transform: (_:Input)=>R): Observable<R> {
      return new Observable<R>(observer=> this.subscribe(e=>observer.next(transform(e)), ()=>observer.complete())
      );
  }
  ...
  scan<V>(initialVal:V, fun: (a:V, el:Input) => V): Observable<V> { 
    return new Observable<V>((observer: Observer<V>) => {
        ...
        return this.subscribe(
            v => {
                try{
                  // try to run the function line 
                  accumulator = fun(accumulator, v)
                }
                catch{
                  //if errors occurs, catch it, and store in observer error, with 
                  // new Error object created
                  observer.error(new Error('Value may be undefined'))
                }
