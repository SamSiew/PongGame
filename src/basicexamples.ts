...
function drawAndDragRectsObservable() {
  ...
  const mousedraw = Observable.fromEvent<MouseEvent>(svg, 'mousedown')
        .map(e=>({event:e, currentbound: svg.getBoundingClientRect()})) //turn the argument e to return e as event and svgBounds to construct shape
        
        //construct the shape rectangle by adjusting x and y accordingly
        .map(({event, currentbound})=> ({
          ...
          .subscribe(({rect,currentbound})=>{
            ...
            Observable.fromEvent<MouseEvent>(svg, 'mousemove') //observable for mousemove so user will adjust the drawing of the rectangle by moving the mouse after doing mouse down
                .takeUntil(Observable.fromEvent(svg, 'mouseup')) //do it until the mouse up. after the mouse is no longer "down", it will finilised the shape of rectangle
                .map(({clientX, clientY})=>({
                    y: clientY - currentbound.top,
                    x: clientX - currentbound.left,
                    rect, X, Y}))
                .subscribe(({ y, x, rect, X, Y})=>
                    rect.attr('x', Math.min(x,X))
                        .attr('y', Math.min(y,Y))
                        .attr('width', Math.abs(X - x))
                        .attr('height', Math.abs(Y - y))
                //drag
                .observe<MouseEvent>('mousedown').forEach(e=>e.stopPropagation())
                .map(({clientX, clientY}) => ({ xOffset: Number(rect.attr('x')) - clientX,
                                                yOffset: Number(rect.attr('y')) - clientY }))
                .flatMap(({xOffset, yOffset}) =>
                  mousemove
